...
export async function selectCategorias() {
    const response = await supabase.from("categorias_dev").select("id, nome, descricao, slug").order('id');
    return response.data;
}

export async function selectCategoriaBySlug(slug) {
    const response = await supabase.from("categorias_dev").select("id, nome, descricao, slug").eq("slug", slug).single();
    return response.data;
}

export async function insertCategorias(nome, descricao) {
    await supabase.from("categorias_dev").insert({nome: nome, descricao: descricao, slug: getSlug(nome)});
}

export async function updateCategorias(id, nome, descricao) {
    const response = await supabase.from("categorias_dev").update({nome: nome, descricao: descricao, slug: getSlug(nome)}).eq("id", id);
    return response.data;
}

export async function deleteCategorias(id) {
    await supabase.from("categorias_dev").delete().eq("id", id);
}
